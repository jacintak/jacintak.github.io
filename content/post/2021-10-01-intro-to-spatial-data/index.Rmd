...
NB: This was originally a tutorial given to Space Lunch members on 6th October 2021. This is an adapted version. The original version can be found on the [Project](project/NERD-club/) page. 
...
- The main `R` package for handling rasters is `raster`. It's a base package.
...
Rasters can be manipulated using base R functions. E.g. addition or subtraction between rasters or layers. Subet rasters by layers using basic square bracket subsetting for lists.
...
Rasters can be manipulated by base functions. There are many other functions for analysing rasters and doing spatial analysis (e.g. interpolation) but we won't cover that here.
...
* Co-ordinates are stored as `decimalLatitude` and `decimalLongitude`. I've removed any missing values.
...
We see that points come mainly from around northern Europe.
...
## What range of water temperatures do house sparrows live in?

We can use our new species distribution points to query the raster and extract values corresponding with the occurrence points. The function to query a raster is `raster::extract`. The same can be used within `tidyverse` via `mutate`.
...
```{r}
hist(bird_temps$temps, main = "Temperature distribution of house sparrow")
```

We can see they live between `r round(min(bird_temps$temps),1)` and `r round(max(bird_temps$temps),1)` &deg;C.
