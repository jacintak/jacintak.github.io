...
authors: []
lastmod: '2022-06-17T16:36:19+01:00'
featured: no
image:
  caption: ''
  focal_point: ''
...
`opts_knit` sets the options for knitting and `find_rstudio_root_file()` is a helper function to get the root directory of the RStudio project (provided you are in one).
...
> Is this really useful? Probably not, but we do it because we can!

***

# Other things

Theoretically, `opts_knit` can be set within your `.Rprofile` but I haven't gotten it to work the way I want it to. `options(knitr.package.root.dir = rprojroot::find_rstudio_root_file())` will set the correct directory but the global `.Rprofile` is not called each time the project is knitted. You could have a project specific `.Rprofile` to your project root directory.
