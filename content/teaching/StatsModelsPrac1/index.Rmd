...
The assumptions we make above are based on a **Type II functional response model** and is the one most appropriate for a predator-prey interaction scenario. You can read about the full derivation by running `vignette("TypeII_models")`. 
...
**For simplicity, we will use a value of 1 minute only so that we can divide by 1 and it makes our maths easier.**
</div>
</div>
</div>
